...
CLOVE_TEST(LinkedList_PopElement)
{
    ...
    CLOVE_PTR_EQ(items[1], linked_list);
    CLOVE_INT_EQ(items[1]->object, linked_list->object);
...
CLOVE_TEST(LinkedList_InvertNullList)
{

    int_item *linked_list = NULL;

    int_item *head = list_invert(&linked_list, int_item);

    CLOVE_NULL(head);
}

CLOVE_TEST(LinkedList_InvertOneElementList)
{

    int_item *linked_list = NULL;

    list_append(&linked_list, item_new(0, int_item), int_item);

    int_item *inverted = list_invert(&linked_list, int_item);

    CLOVE_PTR_EQ(linked_list, inverted);
    CLOVE_INT_EQ(linked_list->object, inverted->object);

    list_destroy(&linked_list, int_item);
}

CLOVE_TEST(LinkedList_InvertTwoElementsList)
{
    ...
    CLOVE_PTR_EQ(tail, inverted);
    CLOVE_INT_EQ(tail->object, inverted->object);

    CLOVE_PTR_EQ(tail->node.next, head);
    CLOVE_NULL(head->node.next);

    list_destroy(&linked_list, int_item);
}

CLOVE_TEST(LinkedList_InvertList)
{
    ...
    CLOVE_PTR_EQ(items[4], inverted);
    CLOVE_INT_EQ(items[4]->object, inverted->object);

    CLOVE_PTR_EQ(linked_list->node.next, items[3]);
    CLOVE_PTR_EQ(items[3]->node.next, items[2]);
    CLOVE_PTR_EQ(items[2]->node.next, items[1]);
    CLOVE_PTR_EQ(items[1]->node.next, items[0]);
    CLOVE_NULL(items[0]->node.next);
...
CLOVE_SUITE(LinkedList)
{
    ...
    CLOVE_SUITE_TESTS(LinkedList_AppendHeadNull, LinkedList_AppendHead, LinkedList_AppendElement, LinkedList_AppendElementFail, LinkedList_GetTail,
                      ...
                      LinkedList_RemoveHead, LinkedList_RemoveTail, LinkedList_InvertNullList, LinkedList_InvertOneElementList, LinkedList_InvertTwoElementsList,
                      LinkedList_InvertList);
}
